...
mkdir oh_god_why
cd oh_god_why
git clone git@github.com:puppetlabs/console.git --depth 1 --branch "{scm_branch}"
cd console
mkdir -p applications/certificates
pushd applications/certificates
...
bundle exec phantomjs ./run_jasmine.coffee http://localhost:${{PORT}}/applications/certificates/spec/runner.html
RESULT=$?
